...
// Parse filename format: YYYY-MM-DD-title.md
...
// Clean markdown syntax (like {:target="_blank"})
...
export function getAllPostSlugs(): string[] {
  ...
  return fileNames
    ...
    .map((name) => parseFilename(name).slug);
}
...
export async function getPostBySlug(slug: string): Promise<Post | null> {
  ...
  const fileName = fileNames.find((name) => {
    ...
    try {
      return parseFilename(name).slug === slug;
    } catch {
      return false;
    }
  });
